...
impl Error {
    ...
    pub(crate) fn get_stack(&self) -> StdString {
        match self {
            Error::Allocation => format!("{:?}", Backtrace::new()),
            Error::InvalidString(_) => format!("{:?}", Backtrace::new()),
            Error::Utf8(_) => format!("{:?}", Backtrace::new()),
            Error::Io { message, backtrace } => format!("{:?}", backtrace),
            Error::Exception {
                message,
                file,
                line,
            ...
            Error::NumArgs { expected, given } => format!("{:?}", Backtrace::new()),
            Error::Resolving {
                base,
                name,
                message,
            } => format!("{:?}", Backtrace::new()),
            Error::Loading { name, message } => format!("{:?}", Backtrace::new()),
            Error::Unknown => format!("{:?}", Backtrace::new()),
        }
    }

    #[cfg(not(feature = "backtrace"))]
    pub(crate) fn get_stack(&self) -> StdString {
        Some("place1".to_string())
    }
    ...
    fn into_js(self, ctx: Ctx<'js>) -> Result<Value<'js>> {
        ...
        match self {
            Exception {
                ...
                if let Some(stack) = rust_stack {
                    println!("Setting rust_stack to {}", stack);
                    value.set("rustStack", stack)?;
                }
            }
            error => {
                value.set("message", error.to_string())?;
                let stack = self.get_stack();
                println!("Setting rust_stack22 to {}", stack);
